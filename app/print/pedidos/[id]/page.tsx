...
export default async function PrintOrderPage({ params, searchParams }: Props) {
  ...
  // HTML + estilos para impressão (A4 e cupom)
  return (
    <html lang="pt-BR">
      <head>
        <title>Imprimir Pedido #{String(order.id).slice(0,8).toUpperCase()}</title>
        ...
        <div className="wrap">
          ...
          <header>
            <div>
              ...
              <div style={{fontSize: mode === 'cupom' ? '11px' : '13px', color: 'var(--muted)'}}>{order.company?.address || ''}</div>
            </div>
            <div className="meta">
              Pedido #{String(order.id).slice(0,8).toUpperCase()}<br />
          ...
          <section className="customer">
            ...
            <div style={{color:'var(--muted)'}}>{order.customer_email}{order.customer_phone ? ` · ${order.customer_phone}` : ''}</div>
            {order.delivery_type === 'DELIVERY' && (
              <div style={{color:'var(--muted)', marginTop:6}}>
                {address?.street || address?.address || '-'}{address?.city ? `, ${address.city}` : ''}{address?.state ? ` - ${address.state}` : ''}{address?.zip_code ? ` · CEP ${address.zip_code}` : ''}
              </div>
            )}
            <div style={{color:'var(--muted)', marginTop:6}}>
          ...
          <section>
            ...
            <div className="totals">
              <div className="row"><span>Subtotal</span><strong>{formatMoney(Number(order.subtotal ?? 0))}</strong></div>
              <div className="row"><span>Taxa de Entrega</span><strong>{formatMoney(Number(order.delivery_fee ?? 0))}</strong></div>
              <div className="row" style={{fontSize: mode === 'cupom' ? '13px' : '16px'}}><span>Total</span><strong>{formatMoney(Number(order.total_amount ?? order.total_amount ?? 0))}</strong></div>
            </div>

            {order.notes && <div className="notes">Obs: {order.notes}</div>}
          </section>

          <footer style={{marginTop: 12, textAlign: 'center', color: 'var(--muted)', fontSize: mode === 'cupom' ? '11px' : '13px'}}>Obrigado pela preferência!</footer>
        </div>
